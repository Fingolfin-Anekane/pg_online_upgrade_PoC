...
func TestErrorClass(t *testing.T) {
	...
	// errors.As must unwrap a wrapped PgError, not fall through to string match
	assert.Equal(t, "read-only", errorClass(fmt.Errorf("driver: %w", &pgconn.PgError{Code: "25006"})))
}
