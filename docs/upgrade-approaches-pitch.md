...
> кластера на параллельный, откат до cutover тривиален. Цена — отдельный параллельный кластер и
...
Затем — **перенос 2 реплик в новый кластер** (TODO, пока вручную): на
...
**Закладки:**
- [TODO] **Формирование HA нового кластера** пока вне тулзы: она поднимает
  только лидера, перенос/добавление реплик (`pg-main-4`, `pg-main-5`) — отдельный
  шаг; `VerifyNewClusterHealthy` лишь напоминает («есть лидер, реплик пока
...
**Закладки:**
- [внешнее] Сам DSN-swap делает внешний прокси; тулза только пишет сигнал и
...
**Закладки:** нет внешних шагов (предполагаем, что сам shadow-кластер уже
...
**Закладки:**
- [TODO] Чекпойнт перед `upgrade` требует **остановить Patroni на всех
...
**Закладки:**
- [TODO] Реплики шэдоу остановлены (см. B.3) **до** удаления ключа.
- Удаление `/service/pg-main-shadow` — это и есть «магия sysid»: после `pg_upgrade`
  ...
  заново записать `/initialize` с новым sysid **под тем же scope** (scope не
  переименовываем под sysid; финальный rename в `pg-main` — TODO, см. B.8).
...
**Закладки:**
- [TODO] **Ставим `bin_dir=PG17` на репликах** (`pg-shadow-2/3`)
  и поднимаем их Patroni перед фазой — scope у них уже верный (ключ-делит сохранил
...
4. **Сигнал DSN-swap** (`NotifyDSNSwap`): пишем сигнал «новый primary =
   `pg-main-shadow` лидер»; сам swap выполняет внешний прокси (тулза DSN не дёргает).
...
| **Зависимость от ssh/доступов** | локальные операции на ноде N1 | тулза на shadow-лидере + reinit по REST (ssh не нужен); [TODO] рассматривается rsync-fast-path (потребует ssh до реплик) |

---

## 5. Свод TODO и чекпойнтов
...
`rebuild-replicas`, `switchover`, `finalize` — каждый требует явного подтверждения «y» для продолжения.
...
с боевого кластера на параллельный. Откат до cutover тривиален. In-place же держит боевой кластер на паузе с
